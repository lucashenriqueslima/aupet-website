...
if (isset($_GET['id']) AND ! empty($_GET['id'])) {
    $tipos = $sistema->DB_fetch_array("SELECT A.* FROM hbrd_cms_empreendimentos_tipos A INNER JOIN hbrd_cms_empreendimentos_cms_empreendimentos_tipos B ON B.id_tipo = A.id INNER JOIN hbrd_cms_empreendimentos C ON C.id = B.id_empreendimento WHERE C.id_cidade = {$_GET['id']} AND C.stats = 1 GROUP BY A.id ORDER BY A.nome");
} else {
