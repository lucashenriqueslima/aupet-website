...
class __classe__ extends SistemaBase {
    ...
    function __construct()  {
        parent::__construct();
        $this->table = "hbrd_app_motive";
        $this->module_icon = "icomoon-icon-list-2";
        $this->module_title = "Motivos de arquivamento";
    }
    ...
    protected function saveAction() { unset($_POST['delete_at']);
        if(!$_POST['id']) {
            $this->DB_insert($this->table,['nome'=>$_POST['nome']]);
        } else {
            $this->DB_update($this->table,['nome'=>$_POST['nome']]," where id = {$_POST['id']}");
        }
    }
