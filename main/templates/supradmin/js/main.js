...
$(document).ready(function () {
    ...
    $('.btn-filter').click(function(e){
        if(e.handleObj.type != 'click' || $(event.target).context.className.indexOf('btn-filtrar') >= 0 || $(event.target).parents('.groupfilter').length > 0) return;
        $('.groupfilter').hide();
        var div = $(this).children()[0];
        $(div).css("left", this.offsetLeft);
        if(!$(div).is(":visible")){
             $(div).fadeIn(100);
             $(div).find('[placeholder~=Filtrar]').focus();
        }
    })
    $('body').click(function(e){
        if(e.target.className == 'btn-filter' || $(event.target).parents('.groupfilter').length > 0 || !$(event.target).context.className.indexOf || $(event.target).context.className.indexOf('day') >= 0) return;
        $('.groupfilter').hide();
    });
...
function showImages(imagens, descricao){
    if(imagens && !Array.isArray(imagens)) imagens = JSON.parse(imagens);
    var pswpElement = document.querySelectorAll('.pswp')[0];
    var items = imagens.map(x => { return {
        src : '/api/orcamento/imagemregulagem/'+ x,
        title : descricao,
        w: 0,
        h: 0
    }})
...
function imagensOrdemOficina(imagens){
    if(imagens && !Array.isArray(imagens)) imagens = JSON.parse(imagens);
    var pswpElement = document.querySelectorAll('.pswp')[0];
    var items = imagens.map(x => { return {
        src : '/sistema/adm/purchaseorder/imagem/id/'+ x,
        w: 0,
        h: 0
    }})
    var options = {
...
function singlePS(src){
    var pswpElement = document.querySelectorAll('.pswp')[0];
    var items = [{ src : src, w: 0, h: 0 }];
    var options = { index: 0 };
    var gallery = new PhotoSwipe( pswpElement, PhotoSwipeUI_Default, items, options);
    gallery.listen('imageLoadComplete', function (index, item) {
        if (item.h < 1 || item.w < 1) {
