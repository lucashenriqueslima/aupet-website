...
(function ($) {
    ...
    $.supr = function (element, options) {
        ...
        /*
         * VERSÃO 2 implementado por Rafael MODIFICADO POR ALAOR O N.P.
        ...
        plugin.setCurrentNav = function () {
            ...
            if (domain === '') {
                ...
                if (plugin.settings.sideNav.absoluteUrl) {
                    ...
                    var afterDomain_ = window.location.pathname.split('/');
                    var path = window.location.pathname.split('/');
                    service = path[path.length - 5];
                    module = path[path.length - 4];
                    var afterDomain = afterDomain_.pop();
                    afterDomain = afterDomain_.pop() + "/" + afterDomain;
                    ...
                    this.setCurrentClass(navLinks, afterDomain, service+"/"+module);
        ...
        //VERSÃO 3 implementado por Rafael
        //set current nav element
        plugin.setCurrentNav = function (e) {
            var navig = $('.mainnav> ul');
            var navLinks = navig.find('a');

            var afterDomain = window.location.toString().replace(system_path.toString(), '');
            if (plugin.settings.sideNav.subDir != '') {
                var afterDomain = window.location.pathname + plugin.settings.sideNav.subDir;
            }

            var path = afterDomain.split('/');
            service = path[0];
            module = path[1];
