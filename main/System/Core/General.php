...
class General extends \Connector
{
    //debug
    ...
    /*
     * RETORNA ARRAY DE NÚMEROS, NÚMEROS CONTIDOS NUMA STRING
     */
    public function array_numbers($string)
    {
        return array_values(array_filter(explode(",", preg_replace("/[^0-9]/", ",", $string)), 'is_numeric'));
    }
    ...
    { //DEPRECATING
        $query = $this->DB_fetch_array("SELECT IFNULL(C.ler,B.ler) ler, IFNULL(C.gravar,B.gravar) gravar, IFNULL(C.editar,B.editar) editar, IFNULL(C.excluir,B.excluir) excluir FROM hbrd_main_funcoes A INNER JOIN hbrd_main_permissoes B ON A.id=B.id_funcao LEFT JOIN hbrd_main_usuarios_permissoes C ON C.id_funcao = A.id AND C.id_usuario = {$_SESSION['admin_id']} WHERE A.identificador = '$ref' AND B.id_grupo = {$_SESSION['admin_grupo']}");
        if ($query->query) {
            return $query->rows[0];
        } else {
            return "error";
    ...
    {
        $query = $this->DB_fetch_array("SELECT A.nome ,A.nivel, A.acesso, B.ler, B.gravar, B.editar, B.excluir FROM hbrd_main_niveis A INNER JOIN hbrd_main_niveis_permissoes B ON A.id = B.id_nivel INNER JOIN hbrd_main_funcoes C ON B.id_funcao = C.id INNER JOIN hbrd_main_usuarios D ON A.id = D.id_nivel WHERE D.id = '{$_SESSION['admin_id']}' AND C.identificador = '$this->permissao_ref'");
        if ($query->num_rows) {
            $this->level = $query->rows[0];
        } else {
            $this->level["nivel"] = $this->DB_fetch_array("SELECT max(nivel) nivel FROM hbrd_main_niveis")->rows[0]["nivel"] + 1;
            $this->level["ler"] = 0;
            $this->level["gravar"] = 0;
    ...
    function allowedMain()
    {
        if (isset($_SESSION['admin_id'])) {
            $perm = ($_SESSION['admin_perm_serv_ind']) ? $this->DB_fetch_array("SELECT A.*, IF(C.id_servico IS NULL, 0 , 1) permitido FROM hbrd_main_servicos A LEFT JOIN hbrd_main_usuarios_main_servicos C ON A.id = C.id_servico  AND C.id_usuario = '{$_SESSION['admin_id']}' WHERE A.identificador = 'main' GROUP BY A.id")->rows[0]['permitido'] : $this->DB_fetch_array("SELECT A.*, IF(B.id_servico IS NULL, 0 , 1) permitido FROM hbrd_main_servicos A LEFT JOIN hbrd_main_grupos_main_servicos B ON A.id = B.id_servico AND B.id_grupo = '{$_SESSION['admin_grupo']}' WHERE A.identificador = 'main' GROUP BY A.id")->rows[0]['permitido'];
    ...
    function allowedService($service)
    {
    ...
    function allowedServiceLogin($service)
    {
        if (isset($_SESSION['admin_id'])) {
            $perm = ($_SESSION['admin_perm_serv_ind']) ? $this->DB_fetch_array("SELECT A.*, IF(C.id_servico IS NULL, 0 , 1) permitido FROM hbrd_main_servicos A LEFT JOIN hbrd_main_usuarios_main_servicos C ON A.id = C.id_servico  AND C.id_usuario = '{$_SESSION['admin_id']}' WHERE A.identificador = '$service' GROUP BY A.id ORDER BY A.nome")->rows[0]['permitido'] : $this->DB_fetch_array("SELECT A.*, IF(B.id_servico IS NULL, 0 , 1) permitido FROM hbrd_main_servicos A LEFT JOIN hbrd_main_grupos_main_servicos B ON A.id = B.id_servico AND B.id_grupo = '{$_SESSION['admin_grupo']}' WHERE A.identificador = '$service' GROUP BY A.id ORDER BY A.nome")->rows[0]['permitido'];
            return $perm ? true : false;
    ...
    public function getImageFileOriginal($filepath, $sistema = false)
    {
        if (strpos($filepath, 'http') !== false) {
            return $filepath;
        } else {
            if ($sistema) return $this->upload_path . $filepath;
            else return $this->upload_folder . $filepath;
        }
    }
    ...
    function enviarEmail($to = "", $from = "", $assunto = "", $mensagem = "", $bcc = "", $arquivo = "", $arquivos = [])
    {
        ...
        $this->_empresa = $this->DB_fetch_array("SELECT * FROM hbrd_cms_configuracoes_gerais WHERE id = 1")->rows[0];
        $smtp = $query->rows[0];
        $mail = new PHPMailer();
        ...
        if ((bool)count($arquivos)) {
            foreach ($arquivos as $row) {
                $mail->AddAttachment($this->upload_folder . $row['arquivo'], $row['nome']);
            }
        } else if (is_array($arquivo)) {
            $mail->AddAttachment($this->upload_folder . $arquivo['arquivo'], $arquivo['nome']);
        }
        if ($from == "") {
        ...
        $mail->setFrom($smtp['email_padrao'], utf8_decode($this->_empresa['nome']));
        if ($bcc != "") {
            if (is_array($bcc)) {
                foreach ($bcc as $email) {
                    $email['nome'] = utf8_decode($email['nome']);
                    $mail->addBCC($email['email'], $this->_empresa['nome']);
                }
            }
        }
        if (is_array($to)) {
...
//            }
    ...
    function inserirRelatorio($atividade)
    {
        ...
        $this->mysqli->query("INSERT INTO hbrd_main_logs (usuario, atividade, id_usuario, date) VALUES ('$nome','$atividade', $idUsuario,NOW())");
...
//        if(!(bool)$service) $service = __service__;
//        if(!(bool)$classe) $classe = __classe__;
        $this->DB_insert('hbrd_main_bugs', 'erro,service,module,usuario', "'$erro','$service','$classe','{$_SESSION['admin_id']} - {$_SESSION['admin_nome']}'");
        //VERIFICA SE A CONEXÃO FOI ABERTA AGORA PARA DESCONECTAR
    }
    ...
    function checkdate($data)
    {
        ...
        $data = explode("/", $data);
        return (isset($data[0]) and isset($data[1]) and isset($data[2])) ? checkdate($data[1], $data[0], $data[2]) : false;
    }
    ...
    {
        $from = "áàãâèéêíóôõúüçÁÀÃÂÉÊÍÓÔÕÚÜÇ";
        $to = "aaaaeeeiooouucAAAAEEIOOOUUC";
        $keys = array();
        $values = array();
    ...
    {
        $dados = $this->DB_fetch_array("SELECT * FROM hbrd_main_cidades WHERE id_estado='" . $id . "' ORDER BY cidade");
        return $dados->rows;
    }
    ...
    function formataMoedaShowNoDecimal($valor = null)
    {
        if ($valor != null) {
    ...
    public function gerarNoBots()
    {
        ...
        if ($bots) {
            ...
            for ($i = 0; $i < count($bots); $i++) {
                $noBot .= $separador . " LOWER(browser) NOT LIKE '%$bots[$i]%'  ";
                $separador = " AND ";
            }
        }
        return $noBot;
    ...
    public function seoAcessosAtualiza()
    {
        ...
        $verificar = $this->DB_fetch_array("SELECT DATE_FORMAT(data, '%Y-%m-%d') dia FROM hbrd_cms_dashboard_configuracoes WHERE origem = 'general'");
        if ($verificar->num_rows) {
            if ($verificar->rows[0]['dia'] != "")
                $dataVerificada = $verificar->rows[0]['dia'];
        }
        if ($dataVerificada < $diaAnterior) {
            $paginas = $this->DB_fetch_object("SELECT A.id_seo id_seo, COUNT(A.id_seo) qtd, date(date) date FROM hbrd_cms_seo_acessos A WHERE DATE(A.date) > '$dataVerificada' AND DATE(A.date) <= '$diaAnterior' GROUP BY DATE(A.date), A.id_seo");
            if ($paginas->num_rows) {
                foreach ($paginas->rows as $row) {
                    $insertData[] = "($row->id_seo, $row->qtd, '$row->date')";
                }
                if (isset($insertData)) {
                    $query = $this->mysqli->query("INSERT INTO hbrd_cms_dashboard_paginas (id_seo,qtd,date) VALUES " . implode(',', $insertData));
                }
                unset($insertData);
            }
            $visitas = $this->DB_fetch_object("SELECT date, COUNT(session) visitas, COUNT(desktop) desktop, COUNT(tablet) tablet, COUNT(mobile) mobile FROM (SELECT session, DATE_FORMAT(date, '%Y-%m-%d') date, CASE dispositivo WHEN 1 THEN COUNT(dispositivo) END desktop, CASE dispositivo WHEN 2 THEN COUNT(dispositivo) END tablet, CASE dispositivo WHEN 3 THEN COUNT(dispositivo) END mobile FROM hbrd_cms_seo_acessos WHERE DATE(date) > '$dataVerificada' AND DATE(date) <= '$diaAnterior' GROUP BY session) sub GROUP BY date ORDER BY date");
            if ($visitas->num_rows) {
                foreach ($visitas->rows as $row) {
                    $insertData[] = "('$row->date', $row->visitas, $row->desktop, $row->tablet, $row->mobile)";
                }
                if (isset($insertData)) {
                    $query = $this->mysqli->query("INSERT INTO hbrd_cms_dashboard_visitas (date,visitas,desktop,tablet,mobile) VALUES " . implode(',', $insertData));
                }
                unset($insertData);
            }
            $utms = $this->DB_fetch_object("SELECT *, COUNT(session) visitas, COUNT(cadastro) cadastros, COUNT(contato) contatos FROM (SELECT DATE(date) date, 
                ...
                FROM hbrd_cms_seo_acessos A LEFT JOIN (SELECT cadastro, contato, session FROM hbrd_cms_seo_acessos) B ON B.session = A.session
            ...
            if ($utms->num_rows) {
                ...
                if (isset($insertData)) {
                    $query = $this->mysqli->query("INSERT INTO hbrd_cms_dashboard_utms (date,utm_source,utm_medium,utm_term,utm_content,utm_campaign,visitas,cadastros,contatos) VALUES " . implode(',', $insertData));
                }
                unset($insertData);
            }
            $organicos = $this->DB_fetch_object("SELECT date, COUNT(session) visitas , COUNT(cadastro) cadastros, COUNT(contato) contatos FROM (SELECT DATE(date) date, 
               ...
               (SELECT session FROM hbrd_cms_seo_acessos WHERE utm_source != '')
                GROUP BY A.session) B GROUP BY date, utm");
            if ($organicos->num_rows) {
                ...
                if (isset($insertData)) {
                    $query = $this->mysqli->query("INSERT INTO hbrd_cms_dashboard_utms (date,visitas,cadastros,contatos) VALUES " . implode(',', $insertData));
                }
                unset($insertData);
            }
            if ($paginas->num_rows || $visitas->num_rows || $utms->num_rows || $organicos->num_rows) {
                $this->DB_update("hbrd_cms_dashboard_configuracoes", " data = '$diaAnterior 23:59:59' WHERE origem = 'general'");
                $this->DB_delete("hbrd_cms_seo_acessos", " DATE_FORMAT(date, '%Y-%m-%d') <= '$diaAnterior' ");
            }
        }
    }
    ...
    public function DB_backup()
    {
        ...
        $antigos = $this->DB_fetch_array("SELECT *, DATE_FORMAT(data, '%d/%m/%Y') data FROM hbrd_main_backups WHERE DATE(data) < DATE_SUB(CURDATE(), INTERVAL 7 DAY)");
        if ($antigos->num_rows) {
            foreach ($antigos->rows as $antigo) {
                ...
                $this->DB_delete("hbrd_main_backups", "id={$antigo['id']}");
                $this->inserirRelatorio("Apagou backup database antigo: [{$antigo['data']}], id: [{$antigo['id']}]");
            }
        }
        //APAGA BACKUPS SQL COM MAIS DE 1 DIA
        $antigos = $this->DB_fetch_array("SELECT *, DATE_FORMAT(data, '%d/%m/%Y') data FROM hbrd_main_backups WHERE DATE(data) < DATE(CURDATE())");
        if ($antigos->num_rows) {
            foreach ($antigos->rows as $antigo) {
        ...
        for ($i = 0; $i < count($this->dbs); $i++) {
            ...
            if ($finaliza) {
                $this->DB_insert("hbrd_main_backups", "arquivo", "'main/$dir/$arquivo'");
     ...
     * RENDERIZA TELA
     */
    public function renderView($service = 'main', $module = 'dashboard', $view = 'index', $param = array())
    {
        $existe = false;
        $caminho = "environment/$service/module/" . str_replace("-", "_", $module) . "/view/$view.phtml";
     ...
     * RENDERIZA AJAX
     */
    public function renderAjax($service = 'main', $module = 'dashboard', $ajax = 'index', $param = array())
    {
        $existe = false;
        $caminho = "services/$service/modules/" . str_replace("-", "_", $module) . "/ajax/$ajax.phtml";
        if ($this->file_exists_ci($caminho)) {
            require_once $this->file_exists_ci($caminho);
            $existe = true;
        }
     ...
     */
    public function renderExport($service = 'main', $module = 'dashboard', $export = 'index', $param = array())
    {
        $existe = false;
        $caminho = "services/$service/modules/" . str_replace("-", "_", $module) . "/export/$export.phtml";
        if ($this->file_exists_ci($caminho)) {
            require_once $this->file_exists_ci($caminho);
            $existe = true;
        }
    ...
    /*
     * FUNÇÃO PARA CRIAR SESSÕES COM UMA RAIZ DE PREFERENCIA O MODULO, E COMO SEGUNDO PARAMETRO ARRAY COM TODAS AS INFORMAÇÕES DESEJADAS
     */
